import{n as e}from"./glsl-CHAh5tEH.js";import{t}from"./FloatPassUniform-BGNunQwI.js";import{n,t as r}from"./ScreenSizePerspective.glsl-kukqaq0s.js";import{t as i}from"./FloatBindUniform-Dxcf3oBX.js";import{i as a,n as o,r as s,t as c}from"./View.glsl-Cp2_MWNu.js";import{n as l}from"./VerticalOffset.glsl-Dw6FRjLi.js";import{t as u}from"./Float4BindUniform-8ui9MB4P.js";import{t as d}from"./BooleanBindUniform-DyG_e1Kv.js";import{t as f}from"./Texture2DBindUniform-PM77Echq.js";var p=.5;function m(d,f){d.include(n),d.attributes.add(`position`,`vec3`),d.attributes.add(`normal`,`vec3`),d.attributes.add(`centerOffsetAndDistance`,`vec4`);let p=d.vertex;o(p,f),c(p,f),p.uniforms.add(new u(`viewport`,e=>e.camera.fullViewport),new t(`polygonOffset`,e=>e.shaderPolygonOffset),new i(`aboveGround`,e=>e.camera.aboveGround?1:-1)),f.hasVerticalOffset&&l(p),p.code.add(e`struct ProjectHUDAux {
vec3 posModel;
vec3 posView;
vec3 vnormal;
...
float absCosAngle;
  ...
  `),f.draped&&!f.hasVerticalOffset||s(p),f.draped||(p.uniforms.add(new i(`perDistancePixelRatio`,e=>Math.tan(e.camera.fovY/2)/(e.camera.fullViewport[2]/2))),p.code.add(e`
    ...
    `)),f.screenCenterOffsetUnitsEnabled&&a(p),f.hasScreenSizePerspective&&r(p),p.code.add(e`
...
vec2 result = (ij * 2.0 - vec2(1.0)) * clipCoord.w;
return vec4(result, clipCoord.zw);
}`)}function g(t){t.vertex.uniforms.add(new i(`renderTransparentlyOccludedHUD`,e=>e.hudRenderStyle===0?1:e.hudRenderStyle===1?0:.75),new u(`viewport`,e=>e.camera.fullViewport),new f(`hudVisibilityTexture`,e=>e.hudVisibility?.getTexture())),t.vertex.include(h),t.vertex.code.add(e`bool testHUDVisibility(vec4 posProj) {
